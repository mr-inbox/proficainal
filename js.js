...
var card_type_presence_params = { failureMessage: "Le nom du titulaire de la carte est un champ obligatoire" };
var t = {
    'card_type': 'Card Type',
    'supported_cards_more': 'More',
    'supported_cards_less': 'Less',
    errors: {
        'card_number_required': 'Le numéro de carte est un champ obligatoire',
        'card_number_invalid': 'Entrez un numéro de carte valide'
    }
};
    ...
    if (always_display_cvn || isNotBlank(card_type) && (supported_cards[card_type].cvn_display || supported_cards[card_type].cvn_required)) {
        if (card_cvn_length_validator_params === undefined) {
            card_cvn_length_validator_params = {
                ...
                failureMessage: "Entrez un Cryptogramme visuel valide"
            };
        }
...
function set_cvn_required(card_type) {
    ...
    if (always_require_cvn || (isNotBlank(card_type) && supported_cards[card_type].cvn_required)) {
        if (card_cvn_presence_validator_params === undefined) {
            card_cvn_presence_validator_params = { failureMessage: "Cryptogramme visuel est un champ obligatoire" };
        }
...
function initialize_card() {
    ...
    if (!card_detection_available) {
        ...
        if (card_number_validator === undefined) {
            ...
            card_number_validator.add(Validate.Presence, { failureMessage: "Le numéro de carte est un champ obligatoire" });
            card_number_validator.add(Validate.Length, {  minimum: 9, tooShortMessage: "Entrez un numéro de carte valide" });
            card_number_validator.add(Validate.Custom, {  against: function (value, args) {
                ...
                failureMessage: "Entrez un numéro de carte valide"
            });
    ...
    if (card_expiry_month_validator === undefined) {
        ...
        card_expiry_month_validator.add(Validate.Presence, { failureMessage: "Entrez une date d'expiration valide" });
        card_expiry_month_validator.add(Validate.Custom, {
            against: function (value, args) {
                return validate_expiry_date();
            },
            failureMessage: "Entrez une date d'expiration valide"
        });
    }
    ...
    if (card_expiry_year_validator === undefined) {
        ...
        card_expiry_year_validator.add(Validate.Presence, { failureMessage: "Entrez une date d'expiration valide" });
